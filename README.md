# A COMPARISON OF REAL-TIME VIRTUAL BASS ENHANCEMENT ALGORITHMS

**Abstract**: In consumer electronics, the advent of ultra thin devices has raised interest into Virtual Bass Enhancement (VBE) algorithms for enhancing the bass performance of their small-size loudspeakers, since, due to physical limitations, large volume velocities cannot be achieved. Such techniques exploit psychoacoustic effects originated by the sound signal processing happening in the inner-ear and brain. In most applications of interest, VBE algorithms are required to operate in real-time. In this paper, after designing a nonlinear equivalent circuit of a generic time-domain VBE system, we develop discrete-time VBE algorithms suitable to run in real-time. A MUSHRA test is then employed to evaluate the bass enhancement performance of the proposed algorithms using different nonlinear devices and parameter configurations.

All the audio tracks considered for this test are available below.
